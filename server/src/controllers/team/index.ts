...
export const createTeam = async (
  ...
  const { name, position, imageUrl, facebookUrl, linkedInProfile, twitterUrl, description } = req.body;
  ...
  try {
    const newTeamMember = await prisma.team.create({
      data: { name, position, imageUrl, facebookUrl, linkedInProfile, twitterUrl, description },
    });
...
export const getTeam = async (req: Request, res: Response<APIResponse>): Promise<void> => {
  try {
...
export const updateTeam = async (req: Request, res: Response<APIResponse>): Promise<void> => {
  const { id } = req.params;
  const { name, position, imageUrl, facebookUrl, linkedInProfile, twitterUrl, description } = req.body;
...
export const deleteTeam = async (req: Request, res: Response<APIResponse>): Promise<void> => {
  const { id } = req.params;
