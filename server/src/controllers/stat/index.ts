...
export const updateStat = async (
  ...
  try {
    ...
    const updatedStat = await prisma.stat.update({
      where: { id },
      data: {
        registeredCustomers: registeredCustomers ?? existingStat.registeredCustomers,
    ...
    res.status(500).json({
      success: false,
      message: 'An error occurred while updating the stat. Please try again later.',
      error: error.message,
    });
  }
};
...
export const deleteStat = async (
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      message: 'An error occurred while deleting the stat. Please try again later.',
      error: error.message,
    });
  }
};
