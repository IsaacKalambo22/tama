...
const Profile = () => {
  ...
  const [userDetails, setUserDetails] = useState<UserProps | null>(null) // State for user details
  ...
  const fetchUserDetails = async () => {
    if (session?.id && session?.accessToken) {
      try {
        const response = await fetch(`${BASE_URL}/users/${session.id}`, {
          headers: {
  ...
  if (!userDetails) {
    return <div className="text-center">Failed to load user details.</div>
  }
      ...
      <Card className="flex flex-col lg:flex-row gap-8 shadow-none mt-4 p-8">
        <div className="flex flex-col gap-4">
          <AvatarCard
            ...
            imageSrc={userDetails.avatar || "/assets/images/default-avatar.png"}
          />
        </div>
        <div className="flex flex-col gap-4 w-full">
          <PersonalDetails
            userDetails={{
              ...
              district: userDetails.district ?? "Not set",
            }}
          />
