...
const ModalDeleteReports = ({ isOpen, onClose, file }: Props) => {
  ...
  const onSubmit = async () => {
    ...
    try {
      // If there's a file associated with the publication, delete it first from Supabase
      if (file.fileUrl) {
        ...
        if (!deleteResult) {
          console.warn("Failed to delete file from storage, continuing with publication deletion")
        } else {
          console.log("File deleted successfully from Supabase storage")
        }
      }

      const result = await deleteReportAndPublication(
        file.id,
        fullPath,
        pathWithoutAdmin,
        "/admin"
