...
const HomeCarousel = async () => {
  ...
  try {
    ...
    console.error("Failed to fetch home carousel:", error)
    return (
      <div>
        <AddNewHeader name="Home Carousel" buttonName="New Carousel" />
        <p className="text-red-500">
          Failed to load home carousel. Please try again later.
        </p>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      {homeCarousel.length > 0 ? (
        <HomeCarouselList homeCarousel={homeCarousel} />
      ) : (
        <p className="text-gray-500 text-lg mt-5">
          No home carousel available. Create a new carousel to get started!
        </p>
      )}
    </div>
  )
