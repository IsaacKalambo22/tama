...
const variants = {
  ...
  accept: "border border-[2.5px] border-blue-900 bg-blue-900 bg-opacity-10",
  reject: "border border-[2.5px] border-red-600 bg-red-600 bg-opacity-10",
}
...
type InputProps = {
  ...
  dropzoneOptions?: Omit<DropzoneOptions, "disabled" | "accept" | "maxFiles">
}
  ...
  (
    ...
    const {
      ...
      onDrop: (acceptedFiles) => {
        ...
        if (
          maxFiles &&
          (value?.length ?? 0) + acceptedFiles.length > maxFiles
        ) {
          setCustomError(ERROR_MESSAGES.tooManyFiles(maxFiles))
          return
        }
    ...
    return (
      <div className="w-full">
        <div className="flex flex-col gap-2 w-full">
          {value && value?.length < (maxFiles ?? Infinity) && (
            <div className="w-full">
              {/* Main File Input */}
              <div
                {...getRootProps({
