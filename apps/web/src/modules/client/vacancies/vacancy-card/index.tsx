...
import { VacancyProps, VacancyStatus } from "@/lib/api"
...
const formatSalaryToKwacha = (salary: string) => {
  const match = salary.match(/\$(\d+),\d+ - \$(\d+),\d+/)
  if (!match) return salary
  const [_, minSalary, maxSalary] = match
  const minMonthly = Math.round((parseInt(minSalary) * 750) / 12)
  const maxMonthly = Math.round((parseInt(maxSalary) * 750) / 12)
...
const VacancyCard = ({ vacancy }: Props) => {
  ...
  return (
    <Link href={`/news-updates/vacancies/${vacancy.id}`}>
        ...
        <div className="mt-4">
          ...
          <div className="text-sm flex justify-between items-center text-gray-600 mt-4">
            ...
            <span>
              ...
              {formatDate(applicationDeadline.toString())}
            </span>
          </div>
        </div>
      </Card>
