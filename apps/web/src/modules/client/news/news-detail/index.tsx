import { NewsProps, fetchNewsById } from "@/lib/api"
import { formatContent, formatDateTime } from "@/lib/utils"
import Image from "next/image"

export default async function NewsDetail({ id }: { id: string }) {
  const news: NewsProps | null = await fetchNewsById(id) // Fetch news by ID

  if (!news) {
    return (
      <div className="flex flex-col items-center justify-center h-screen">
        <h1 className="text-2xl font-semibold text-gray-700">News Not Found</h1>
        <p className="text-gray-500">
          We couldn&apos;t find the news you were looking for.
        </p>
      </div>
    )
  }
    ...
    <article className="max-w-4xl mx-auto px-6 py-12">
      ...
      <header className="mb-8">
        ...
        <div className="text-sm text-gray-500 mt-2">
          ...
          <span className="ml-4">{formatDateTime(news.createdAt)}</span>
        </div>
      </header>
