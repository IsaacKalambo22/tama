...
export const formatContent = (content: string): string[] => {
  ...
  for (let i = 0; i < sentences.length; i += 5) {
    chunks.push(sentences.slice(i, i + 5).join(" ")) // Group 5 sentences together.
  }
  return chunks
}
...
export const formatCount = (count: string | number) => {
...
export const constructFileUrl = (fileUrl: string) => {
  return fileUrl
}

export const handleDownload = (file: FileProps) => {
  const downloadLink = document.createElement("a")

  downloadLink.href = constructFileUrl(file.fileUrl)
