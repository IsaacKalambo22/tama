...
async function handleFetch<T>(endpoint: string, token?: string): Promise<T> {
  try {
    ...
    if (data.success) {
      ...
      console.log(`API Error at ${endpoint}: ${data.message}`)
...
export const fetchFormsAndDocuments = async (): Promise<FileProps[]> => {
  return handleFetch<FileProps[]>(`${BASE_URL}/forms`)
}

export const fetchShops = async (): Promise<ShopProps[]> => {
  return handleFetch<ShopProps[]>(`${BASE_URL}/shops`)
}

export const fetchEvents = async (): Promise<EventProps[]> => {
  return handleFetch<EventProps[]>(`${BASE_URL}/events`)
}
export const fetchVacancies = async (): Promise<VacancyProps[]> => {
  return handleFetch<VacancyProps[]>(`${BASE_URL}/vacancies`)
}
export const fetchServices = async (): Promise<ServiceProps[]> => {
  return handleFetch<ServiceProps[]>(`${BASE_URL}/services`)
}

export const fetchBlogs = async (): Promise<BlogProps[]> => {
  return handleFetch<BlogProps[]>(`${BASE_URL}/blogs`)
}

export const fetchBlogById = async (id: string): Promise<BlogProps> => {
  return handleFetch<BlogProps>(`${BASE_URL}/blogs/${id}`)
}
export const fetchVacancyById = async (id: string): Promise<VacancyProps> => {
  return handleFetch<VacancyProps>(`${BASE_URL}/vacancies/${id}`)
}

export const fetchNews = async (): Promise<NewsProps[]> => {
  return handleFetch<NewsProps[]>(`${BASE_URL}/news`)
}

export const fetchNewsById = async (id: string): Promise<NewsProps> => {
  return handleFetch<NewsProps>(`${BASE_URL}/news/${id}`)
}

export const fetchCouncilList = async (): Promise<CouncilListProps[]> => {
