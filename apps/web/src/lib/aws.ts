...
const s3Client = new S3Client({
  ...
  credentials: {
    ...
    secretAccessKey: config.env.aws.secretAccessKey,
  },
})
...
export const getSignedURL = async ({
  ...
  try {
    ...
    const sanitizedFileName = fileName.replace(/\s*-\s*|\s+/g, "-")
...
export const removeFromS3 = async (urls: string | string[]) => {
  try {
    const session = await auth()
    ...
    return {
      success: deleteResponses.every((res) => res.success),
      details: deleteResponses,
    }
  } catch (error) {
    console.error("Unknown error occurred while deleting from S3:", error)
    return {
      success: false,
      error: error instanceof Error ? error.message : error,
    }
