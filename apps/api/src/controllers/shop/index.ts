...
export const createShop = async (
  ...
  const { name, imageUrl, address, openHours, size } = req.body

  // Validate input
  if (!name || !imageUrl || !address || !size || !openHours) {
    res.status(400).json({
      success: false,
...
export const updateShop = async (
  ...
  try {
    ...
    const updatedData: any = {
      name: name?.trim() === "" ? existingShop.name : name,
      imageUrl: imageUrl?.trim() === "" ? existingShop.imageUrl : imageUrl,
      address: address?.trim() === "" ? existingShop.address : address,
      openHours: openHours?.trim() === "" ? existingShop.openHours : openHours,
    }
