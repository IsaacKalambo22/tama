...
export const createCarousel = async (
  ...
  const { title, coverUrl, description } = req.body

  // Validate input
  if (!title || !coverUrl || !description) {
    res.status(400).json({
      success: false,
      message: "Title, description, & cover URL are required.",
...
export const getHomeData = async (
  ...
  try {
    const carousel = await prisma.carousel.findMany()
    const imageText = await prisma.imageText.findMany()

    res.status(200).json({
      success: true,
      message: "Carousel & image text retrieved successfully",
      data: { carousel, imageText },
    })
  } catch (error: any) {
    console.error("Error fetching carousel & image text:", error.message)
    res.status(500).json({
      success: false,
      message:
...
export const updateCarousel = async (
  ...
  try {
    ...
    const updatedData: any = {
      title: title?.trim() || existingCarousel.title,
      description: description?.trim() || existingCarousel.description,
      coverUrl: coverUrl?.trim() || existingCarousel.coverUrl,
    }
...
export const createImageText = async (
  ...
  const { heading, imageUrl, description } = req.body

  // Validate input
  if (!heading || !imageUrl || !description) {
    res.status(400).json({
      success: false,
      message: "Heading, description, & cover URL are required.",
      error: "Validation error",
    })
    return
  }
  ...
  try {
    ...
    const newImageText = await prisma.imageText.create({
      data: {
        heading,
        imageUrl,
        description,
    ...
    console.error("Error creating imageText:", error.message)
    res.status(500).json({
      success: false,
      message:
...
export const getImageText = async (
  ...
  try {
    ...
    console.error("Error fetching imageText:", error.message)
    res.status(500).json({
      success: false,
      message:
...
export const updateImageText = async (
  ...
  try {
    const existingImageText = await prisma.imageText.findUnique({
      where: { id },
    })
    ...
    const updatedData: any = {
      heading: heading?.trim() || existingImageText.heading,
      description: description?.trim() || existingImageText.description,
      imageUrl: imageUrl?.trim() || existingImageText.imageUrl,
    }

    const updatedImageText = await prisma.imageText.update({
      where: { id },
      data: updatedData,
    })
    ...
    console.error("Error updating imageText:", error.message)
    res.status(500).json({
      success: false,
      message:
...
export const deleteImageText = async (
  ...
  try {
    ...
    console.error("Error deleting imageText:", error.message)
    res.status(500).json({
      success: false,
      message:
