...
export const createReportAndPublication = async (
  ...
  const { fileUrl, filename, size, type, extension } = req.body
  // Validate input
  if (!fileUrl || !filename || !size || !type || !extension) {
    res.status(400).json({
      success: false,
      message: "fileUrl, filename, type, extension and size are required.",
  ...
  try {
    ...
    const newReportAndPublication = await prisma.reportAndPublication.create({
      data: {
        fileUrl,
        filename,
        type,
    ...
    res.status(201).json({
      success: true,
      message: "ReportAndPublication created successfully",
      data: newReportAndPublication,
    })
  } catch (error: any) {
    console.error("Error creating reportAndPublication:", error.message)
    res.status(500).json({
      success: false,
      message:
...
export const getAllReportsAndPublications = async (
  req: Request,
  res: Response<APIResponse>
): Promise<void> => {
  try {
    // Fetch all reportAndPublication from the database
    const reportAndPublication = await prisma.reportAndPublication.findMany({
    ...
    res.status(200).json({
      success: true,
      message: "ReportAndPublication retrieved successfully",
      data: reportAndPublication,
    })
  } catch (error: any) {
    console.error("Error fetching reportAndPublication:", error.message)
    res.status(500).json({
      success: false,
      message:
...
export const updateReportAndPublication = async (
  ...
  const { id } = req.params
  const { filename, fileUrl, size, type, extension } = req.body

  // Validate input
  if (!id) {
    res.status(400).json({
      success: false,
      message: "ReportAndPublication ID is required.",
      error: "Validation error",
    })
    return
  }
  ...
  try {
    ...
    if (!existingReportAndPublication) {
      res.status(404).json({
        success: false,
        message: "ReportAndPublication not found.",
      })
      return
    }
    ...
    const updatedData: any = {
      filename:
        filename?.trim() === ""
          ...
          : filename,
      fileUrl:
        fileUrl?.trim() === "" ? existingReportAndPublication.fileUrl : fileUrl,
      type: type?.trim() === "" ? existingReportAndPublication.type : type,
      extension:
        extension?.trim() === ""
    ...
    res.status(200).json({
      success: true,
      message: "ReportAndPublication updated successfully",
      data: updatedReportAndPublication,
    })
  } catch (error: any) {
    console.error("Error updating reportAndPublication:", error.message)
    res.status(500).json({
      success: false,
      message:
...
export const deleteReportAndPublication = async (
  ...
  try {
    ...
    res.status(200).json({
      success: true,
      message: "ReportAndPublication deleted successfully",
    })
  } catch (error: any) {
    console.error("Error deleting reportAndPublication:", error.message)
    res.status(500).json({
      success: false,
      message:
