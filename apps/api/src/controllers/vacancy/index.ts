...
export const getVacancyById = async (
  ...
  try {
    ...
    console.error("Error fetching vacancy by ID:", error.message)
    res.status(500).json({
      success: false,
      message:
...
export const updateVacancy = async (
  ...
  try {
    ...
    const updatedData = {
      title: title?.trim() === "" ? existingVacancy.title : title,
      description:
        description?.trim() === "" ? existingVacancy.description : description,
      company: company?.trim() === "" ? existingVacancy.company : company,
      location: location?.trim() === "" ? existingVacancy.location : location,
      status: status?.trim() === "" ? existingVacancy.status : status,
      applicationDeadline: applicationDeadline
        ? new Date(applicationDeadline)
        : existingVacancy.applicationDeadline,
      salary: salary?.trim() === "" ? existingVacancy.salary : salary,
      duties: duties?.trim() === "" ? existingVacancy.duties : duties,
      qualifications:
        qualifications?.trim() === ""
          ? existingVacancy.qualifications
          : qualifications,
      howToApply:
        howToApply?.trim() === "" ? existingVacancy.howToApply : howToApply,
    }
