...
export const createEvent = async (
  ...
  const { title, description, date, endDate, time, location } = req.body
  ...
  try {
    const newEvent = await prisma.event.create({
      data: {
        ...
        endDate: endDate ? new Date(endDate) : undefined, // Optional end date
  ...
  try {
    ...
    const updatedEvent = await prisma.event.update({
      ...
      data: {
        ...
        time: time?.trim() || existingEvent.time,
