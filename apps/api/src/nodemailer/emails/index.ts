...
export const setPasswordRequestEmail = async (
  ...
  try {
    const emailBody = SET_PASSWORD_REQUEST_TEMPLATE.replace(
      "{setPasswordURL}",
      setPasswordURL
    )
...
export const sendPasswordResetEmail = async (
  ...
  try {
    const emailBody = PASSWORD_RESET_REQUEST_TEMPLATE.replace(
      "{resetURL}",
      resetURL
    )
    const result = await sendEmail({
...
export const sendContactEmail = async (
  ...
  try {
    const contactEmailBody = CONTACT_EMAIL_TEMPLATE.replace(
      /{senderName}/g,
      senderName
    )
